...
These factors rule out any naïve approach to locality, but there's an elegant solution known as a http://en.wikipedia.org/wiki/Voronoi_diagram[Voronoi diagram].
...
TODO: above paragraph not very clear, may not be necessary.
...
* Use the weather station locations to define a set of Voronoi polygons. Treat each weather station's observations as applying uniformly to the whole of that polygon.
* Choose a lower-bound zoom level that avoids skew but doesn't balloon the dataset's size.
* Break the Voronoi polygons up by quadtile as we did above -- quadtiles will either contain a piece of boundary (and so are at the lower-bound zoom level), or are entirely contained within a boundary.
