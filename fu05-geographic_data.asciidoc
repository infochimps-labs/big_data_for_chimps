...
You can look at quadtiles is as a tree structure

* Each branch splits the plane exactly in half by area
* Only leaf nodes hold data
...
We also have a severe version of the multiscale problem: 

* The coverage varies wildly over space: a similar number of weather stations cover a single large city as cover the entire Pacific ocean.
* The coverage varies wildly over time. In the 1970s, the closest weather station to Austin, TX was about 150 km away in San Antonio. Now, there are dozens in Austin alone. 

These factors rule out any naïve approach to locality, but there's an elegant solution.
...
It's straightforward to draw the right picture.
connect each point with a line to its neighbors, dividing the plane into triangles; there's an efficient alorithm (http://en.wikipedia.org/wiki/Delaunay_triangulation[Delaunay Triangulation]) to do so optimally.

If I stand at the midpoint of the edge connecting two locations, and walk perpendicular to the edge in either direction, I will remain equidistant from each point. 

Extending these lines defines the Voronoi diagram -- a set of polygons, one per point, enclosing the area closer to that point than any other. Locations on the boundary of each Voronoi polygon are equidistant from the two points that define the area on either side.

Hooray! Now our path is clear:

* Use the weather station locations to define a set of Voronoi polygons. We will treat each weather station's observations as applying uniformly to the whole of that polygon.
* Break those polygons up by quadtile, as we did above, to produce quadtiles that are either contain a piece of boundary (in which case they are at the lower-bound zoom level), or are entirely contained within a boundary.
* Also produce the reverse mapping, from weather station to the quadtile IDs its polygon covers.

footnote:[http://www.cs.cornell.edu/home/chew/Delaunay.html[Voronoi Diagram applet] -- requires a Java-enabled browser]

footnote:[John Snow, the father of epidemiology, mapped cholera cases from an 1854 outbreak against the voronoi regions defined by each neighborhood's closest water pump. The resulting infographic made plain to contemporary physicians and officials that bad drinking water, not "miasma" (bad air), transmitted cholera. http://johnsnow.matrix.msu.edu/book_images12.php]
...
**Exercise 2**: Extend quadtile mapping to three dimensions
