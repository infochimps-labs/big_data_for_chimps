...
You can now `LOAD`, `STORE`, `FILTER`, `UNION`, `SPLIT`, `LIMIT` and `FOREACH`/`GENERATE`. This gives you a powerful tool chest with which to modify individual records. The really interesting applications, however, come when we put data into context by grouping similar records. This is the subject of the next chapter.
