...
JOEMAN:Comment Consider adding something like the paragraph below - you can help set the stage even further by giving me a preview of what the C&ECC is going to do for Santa and friends.

Through their prior work at [Google? Large scale e-commerce company?], the Chimpanzee & Elephant Computing Company have developed a breakthrough business process for the chunking and distribution of large tasks.
...
As the process and pass on the toy request, the chimpanzees file them in the order of Santa's path through the world. This is easy, because the files never grow very large and because chimpanzees are very dextrous. So when a pygmy elephant trundles off, all the `puppy` requests are together in order from Auckland to Samoa, and the `robot` requests are together, also in order, and so on:
