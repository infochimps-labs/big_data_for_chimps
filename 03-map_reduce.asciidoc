...
In order to calculate the time delay in reporting UFOs, we've got to determine that delay by subtracting the time the UFO was sighted, from the time the UFO was reported. This occurs in the map phase of our map/reduce job. The mapper emits the time delay in days, and a counter - that is always one.
...
Always test locally on a sample of data, if at all possible. We can execute our map/reduce job locally, like so:
...
Note that we've added a sort to the end - to show the lowest values first. On Hadoop, this would take another map/reduce job.
...
We can execute our mapreduce job on Hadoop using Hadoop Streaming, like so:
