DROP TABLE IF EXISTS `ezevents`;
CREATE TABLE         `ezevents` (
...
SELECT * FROM `ezevents`
  ORDER BY year_id DESC, game_id ASC, inn ASC, inn_home ASC, outs_ct ASC
  INTO OUTFILE '/data/rawd/sports/baseball/retrosheet/events_lite.tsv'
  ;



-- score differential
-- @ev_runs_ct := IF(bat_dest_id > 3, 1, 0) + IF(run1_dest_id > 3, 1, 0) + IF(run2_dest_id > 3, 1, 0) + IF(run3_dest_id > 3, 1, 0) AS ev_runs_ct,
--  @beg_scdiff    := home_score_ct - away_score_ct AS beg_scdiff,
--  @end_scdiff    := @beg_scdiff + IF(bat_home_id = 1, event_runs_ct, -event_runs_ct) AS end_scdiff
