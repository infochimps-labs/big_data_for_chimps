...
For our next example, let's look at geographic data:

* boundaries and metadata for countries, states and counties ("admin 1"/"admin 2") and postal codes.
* the Geonames dataset of places

To retrieve information about each region, no magic is called for: the row key is a compound of the region type and region id, and store the full serialized record as the value. It's important to keep row keys _short_ and _sortable_, so map the region types to single-byte ids (say, `0` for country, `1` for admin 1, etc) and use standard ISO identifiers for the region id (`us` for the USA, `dj` for Djibouti, etc).

The tile-boundaries table is more interesting.
We want to serve boundaries out in tiles:

The tile-places table is most interesting of all.
...
| tile_places   | `quadkey`      	    | (feature class)	| `geonames_id`     | name                  	| `VERSIONS => 1, COMPRESSION => 'SNAPPY'` 
|=======

==== Column Families enable Vertical Partitioning ====

Retrieval from the tile store is always by row, but
on some reads only want a few region types (eg country and state; or state, county and census block)
on others, want many region types for a given country.

This suggests we put each region type in its own "column family"

The HBase API unfortunately makes column families seem far more interesting than they are.
Only want a few; not such a problem on reads, def. a problem on writes.
Want them to have similar size and pattern of write access.

for given country and tile, retrieve all region types
for given tile, retrieve country and state region types 
