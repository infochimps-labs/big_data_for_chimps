...
There's another reason HBase is a great match for

HBase stores all rows in sorted order on disk. This means that when the user has typed *+chim+*, the rows for `chime` and `chimp` are so forth nearby on disk -- the operating system is likely to have the right block hot in its 

The Autocomplete table is a candidate for block-level compression. HBase stores 
