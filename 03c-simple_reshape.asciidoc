...
But there are thousands of places to visit, and so the reindeer want to travel efficiently. We'll use a 'secondary sort' -- a sub-ordering of records within each group -- based on the location.
// In particular, we're going to use the 'quadkey'
