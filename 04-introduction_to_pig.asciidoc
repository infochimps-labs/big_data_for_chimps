...
year_month_count = FOREACH sightings GENERATE SUBSTRING(sighted_at, 5, 7) AS year_month;

-- Group by year_month, and then count the size of the 'bag' this creates to get a total
ufos_by_month    = FOREACH (GROUP year_month_count BY year_month) GENERATE
  group AS year_month, COUNT_STAR(year_month_count);
