...
JT and Nanette were enjoying the rising success of C&E Corp. The translation project was in full production, and they'd made a significant further breakthrough for a major toy distributor (of which more soon).
...
== (TODO: add earlier in the chapter)
...
We can now properly introduce you to the first interesting Pig command: the `FOREACH`, which makes simple transformations to each record. 



A `FOREACH` on its own leads to a mapper-only job, with exactly the same count of output records as input records.  
